...
```{r}
data.bureau.clean %>% summarise_all(funs(100*mean(is.na(.))))
```
```{r}
data.bureau_balance.clean %>% summarise_all(funs(100*mean(is.na(.))))
```

We look at those features in detail.
```{r}
#filter the NA patterned features
byNA.data.bureau.clean <- data.bureau.clean %>% 
